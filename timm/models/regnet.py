...
from .layers import ClassifierHead, AvgPool2dSame, ConvNormAct, SEModule, DropPath, GroupNormAct, cbam, eca, gather_excite
...
class Bottleneck(nn.Module):
    ...
    def __init__(
            ...
            downsample='conv1x1', linear_out=False, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d, attn_layer = None,
       ...
       # se_ratio = 1
        if attn_layer == 'se':
            att_ratio = .5
            att_channels = int(round(in_chs * att_ratio))
            self.att = SEModule(bottleneck_chs, rd_ratio=1, act_layer=act_layer)
            
        elif attn_layer == 'cbam':
            att_ratio = .5
            att_channels = int(round(in_chs * att_ratio))
            self.att = cbam.CbamModule(bottleneck_chs, reduction_ratio=16)
            
        elif attn_layer == 'eca':
            att_ratio = .5
            att_channels = int(round(in_chs * att_ratio))
            self.att = eca.EcaModule(bottleneck_chs, rd_ratio=1, act_layer=act_layer)
            
        elif attn_layer == 'ceca':
            self.att = eca.CecaModule(bottleneck_chs, rd_ratio=1, act_layer=act_layer)
        elif attn_layer == 'ge':
            self.att = gather_excite.GatherExcite(bottleneck_chs, rd_ratio=.5, act_layer=act_layer)
        else:
...
class PreBottleneck(nn.Module):
    ...
    def __init__(
            self, in_chs, out_chs, stride=1, dilation=(1, 1), bottle_ratio=1, group_size=1, se_ratio=0.25,
            downsample='conv1x1', linear_out=False, act_layer=nn.LeakyReLU, norm_layer=nn.BatchNorm2d,
            drop_block=None, attn_layer=None, drop_path_rate=0.):
        ...
        if attn_layer == 'se':
            ...
            #att_ratio = .25
            #att_channels = int(round(in_chs * att_ratio))
            self.att = cbam.CbamModule(bottleneck_chs, reduction_ratio=8, act_layer=act_layer)
            ...
            self.att = gather_excite.GatherExcite(bottleneck_chs, rd_ratio=1, act_layer=act_layer)
        else:
            self.att = nn.Identity()
...
class RegNet(nn.Module):
    ...
    def __init__(
            ...
            drop_rate=0., drop_path_rate=0., zero_init_last=True, block_args=None):
        super().__init__()
        ...
        for i, stage_args in enumerate(per_stage_args):
            ...
            self.add_module(stage_name, RegStage(in_chs=prev_width, block_fn=block_fn, **stage_args, **common_args, **block_args))
...
### SE Models ### 
@register_model
def SEregnetx_016(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='se'), **kwargs)
    return _create_regnet('regnetx_016', pretrained,  **model_args)
    
   
@register_model
def SEregnetx_032(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='se'), **kwargs)
    return _create_regnet('regnetx_032', pretrained,  **model_args)
    
@register_model
def SEregnetx_040(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='se'), **kwargs)
    return _create_regnet('regnetx_040', pretrained,  **model_args)
    
@register_model
def SEregnetx_064(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='se'), **kwargs)
    return _create_regnet('regnetx_064', pretrained,  **model_args)
    
### CBAM Models ###    
@register_model
def CBAMregnetx_016(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='cbam'), **kwargs)
    return _create_regnet('regnetx_016', pretrained,  **model_args)
    
@register_model
def CBAMregnetx_032(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='cbam'), **kwargs)
    return _create_regnet('regnetx_032', pretrained,  **model_args)
...
@register_model
def CBAMregnetx_040(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='cbam'), **kwargs)
    return _create_regnet('regnetx_040', pretrained,  **model_args)
...
@register_model
def CBAMregnetx_064(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='cbam'), **kwargs)
    return _create_regnet('regnetx_064', pretrained,  **model_args)
...
### ECA Models ###    
@register_model
def ECAregnetx_016(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
...
@register_model
def ECAregnetx_032(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='eca'), **kwargs)
    return _create_regnet('regnetx_032', pretrained,  **model_args)
...
@register_model
def ECAregnetx_040(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='eca'), **kwargs)
    return _create_regnet('regnetx_040', pretrained,  **model_args)
...
@register_model
def ECAregnetx_064(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='eca'), **kwargs)
    return _create_regnet('regnetx_064', pretrained,  **model_args)
...
@register_model
def CECAregnetx_040(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='ceca'), **kwargs)
    return _create_regnet('regnetx_040', pretrained,  **model_args)
    
@register_model
def CECAregnetx_064(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='ceca'), **kwargs)
    return _create_regnet('regnetx_064', pretrained,  **model_args) 
...
@register_model
def GEregnetx_040(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='ge'), **kwargs)
    return _create_regnet('regnetx_040', pretrained,  **model_args)
    
@register_model
def GEregnetx_064(pretrained=False, **kwargs):
    """RegNetX-3.2GF"""
    model_args = dict(
        block_args=dict(attn_layer='ge'), **kwargs)
    return _create_regnet('regnetx_064', pretrained,  **model_args)
