...
class DenseLayer(nn.Module):
    def __init__(
        ...
        if attn_layer == 'se':
            ...
            #att_ratio = .5
            #att_channels = int(round(num_input_features * att_ratio))
            self.att = cbam_layer(growth_rate, reduction_ratio=16)
...
@register_model
def CBAMdensenet121(pretrained=False, **kwargs):
...
@register_model
def CBAMdensenet121d(pretrained=False, **kwargs):
    r"""Densenet-121 model from
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`
    """
    model = _create_densenet(
...
@register_model
def CBAMdensenet169(pretrained=False, **kwargs):
...
@register_model
def CBAMdensenet201(pretrained=False, **kwargs):
...
@register_model
def CBAMdensenet161(pretrained=False, **kwargs):
