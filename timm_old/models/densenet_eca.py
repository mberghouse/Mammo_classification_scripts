...
from .layers import BatchNormAct2d, create_norm_act_layer, BlurPool2d, create_classifier, cbam, eca ###########
...
class DenseLayer(nn.Module):
    def __init__(
            ...
            drop_rate=0.,se_layer=SEModule,cbam_layer=cbam.CbamModule, eca_layer=eca.EcaModule,memory_efficient=False):
        ...
        if attn_layer == 'se':
            ...
            self.att = se_layer(growth_rate, rd_ratio=1, act_layer=act_layer)
            ...
            #att_ratio = .5
            #att_channels = int(round(num_input_features * att_ratio))
            self.att = eca_layer(growth_rate, rd_ratio=.125, act_layer=act_layer)
        else:
...
@register_model
def ECAdensenet121(pretrained=False, **kwargs):
...
@register_model
def ECAdensenet121d(pretrained=False, **kwargs):
    r"""Densenet-121 model from
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`
    """
    model = _create_densenet(
...
@register_model
def ECAdensenet169(pretrained=False, **kwargs):
...
@register_model
def ECAdensenet201(pretrained=False, **kwargs):
...
@register_model
def ECAdensenet161(pretrained=False, **kwargs):
