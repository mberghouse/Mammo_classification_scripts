...
from .layers import BatchNormAct2d, create_norm_act_layer, BlurPool2d, create_classifier, cbam, eca, squeeze_excite,gather_excite ###########
...
class DenseLayer(nn.Module):
    def __init__(
            self, num_input_features, growth_rate, bn_size, norm_layer=BatchNormAct2d,act_layer=nn.LeakyReLU,
            drop_rate=0.,ge_layer=gather_excite.GatherExcite,cbam_layer=cbam.CbamModule, eca_layer=eca.EcaModule,memory_efficient=False):
        super(DenseLayer, self).__init__()
        self.add_module('norm1', norm_layer(num_input_features)),
        self.add_module('conv1', nn.Conv2d(
            num_input_features, bn_size * growth_rate, kernel_size=1, stride=1, bias=False)),
        #self.add_module('eca',eca_layer(bn_size*growth_rate)),
        self.add_module('norm2', norm_layer(bn_size * growth_rate)),
        self.add_module('conv2', nn.Conv2d(
            bn_size * growth_rate, growth_rate, kernel_size=3, stride=1, padding=1, bias=False)),
        #self.add_module('se',se_layer(growth_rate,reduction=1)),
        attn_layer = 'ge'
        if attn_layer == 'ge':
            #att_ratio = .5
            #att_channels = int(round(num_input_features * att_ratio))
            self.att = ge_layer(growth_rate, rd_ratio=.25, act_layer=act_layer)
            
        elif attn_layer == 'cbam':
            #att_ratio = .5
            #att_channels = int(round(num_input_features * att_ratio))
            self.att = cbam_layer(growth_rate, rd_ratio=1, act_layer=act_layer)
            
        elif attn_layer == 'eca':
            #att_ratio = .5
            #att_channels = int(round(num_input_features * att_ratio))
            self.att = eca_layer(growth_rate, rd_ratio=1, act_layer=act_layer)
        else:
            self.att = nn.Identity()
        #self.add_module('cbam',cbam_layer(growth_rate,rd_ratio=.125)),
        #self.add_module('eca',eca_layer(growth_rate)),
...
class DenseBlock(nn.ModuleDict):
    ...
    def __init__(
            self, num_layers, num_input_features, bn_size, growth_rate, norm_layer=BatchNormAct2d, se_layer=SEModule,
...
class DenseNet(nn.Module):
    ...
    def __init__(
            ...
            bn_size=4, stem_type='', norm_layer=BatchNormAct2d, se_layer=SEModule, aa_layer=None, drop_rate=0,
...
@register_model
def SEdensenet121(pretrained=False, **kwargs):
...
@register_model
def SEdensenet121d(pretrained=False, **kwargs):
    r"""Densenet-121 model from
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`
    """
    model = _create_densenet(
...
@register_model
def GEdensenet169(pretrained=False, **kwargs):
...
@register_model
def SEdensenet201(pretrained=False, **kwargs):
...
@register_model
def SEdensenet161(pretrained=False, **kwargs):
